...
evolver::evolver(bool _with_cuda, int _sx, int _sy, float _dx, float _dy, float _dt, int _writeEveryNSteps) : sx(_sx), sy(_sy), dx(_dx), dy(_dy), dt(_dt), writeEveryNSteps(_writeEveryNSteps)
{
    std::srand(1);
    #ifdef WITHCUDA
